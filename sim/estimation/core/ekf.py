"""Generic Euclidean-state EKF engine."""
...
from .base import EuclideanProcessModel, EuclideanStateProtocol, MeasurementModel
...
EuclideanStateT = TypeVar("EuclideanStateT", bound=EuclideanStateProtocol)


class GenericEKF(Generic[EuclideanStateT, ControlT, MeasurementT]):
    """Reusable EKF engine over Euclidean nominal states."""

    def __init__(
        self,
        *,
        process_model: EuclideanProcessModel[EuclideanStateT, ControlT],
        initial_state: EuclideanStateT,
        initial_covariance: Matrix,
        covariance_convention: CovarianceConvention | None = None,
        measurement_gate: MeasurementGate[EuclideanStateT, MeasurementT] | None = None,
    ) -> None:
    ...
    def predict(self, control: ControlT, dt: float) -> PredictionDiagnostics:
        ...
        transition_jacobian = np.asarray(
            self.process_model.state_jacobian(prior_state, control, dt),
            dtype=float,
        )
    ...
    def update(
        self,
        *,
        measurement_model: MeasurementModel[EuclideanStateT, MeasurementT],
        ...
        state_correction = kalman_gain @ innovation
        ...
        self.covariance = self._regularize_covariance(updated_covariance, after_update=True)
